...
const details_1 = __webpack_require__(/*! ../models/details */ "./src/models/details.ts");
const path_1 = __importDefault(__webpack_require__(/*! path */ "path"));
    ...
    Create(name, type, agent, date, referenceDate, value, description, details) {
        try {
            ...
            if (details && details.length > 0) {
                detailsObj = new details_1.Details(null, details);
                detailsId = detailsObj.getInfo().id;
                const detailsFilePath = path_1.default.join(this._detailsPath, `${detailsId}.csv`);
                const detailsCSV = (0, sync_2.stringify)(details, { header: true });
                fs.writeFileSync(detailsFilePath, detailsCSV, 'utf-8');
            }
            const transaction = new transaction_1.Transaction(name, description, transactionType, agent, transactionDateTicks, referenceDateTicks, value, detailsObj);
    ...
    Read() {
        try {
            ...
            for (const record of records) {
                let detailsObj = null;
                if (record.details) {
                    const detailsFilePath = path_1.default.join(this._detailsPath, `${record.details}.csv`);
                    if (fs.existsSync(detailsFilePath)) {
                        const detailsContent = fs.readFileSync(detailsFilePath, 'utf-8');
                        const detailsRecords = (0, sync_1.parse)(detailsContent, {
                            columns: true,
                            skip_empty_lines: true
                        });
                        detailsObj = new details_1.Details(record.details, detailsRecords);
                    }
                }
                const transaction = new transaction_1.Transaction(record.name, record.description, record.type, record.agent, parseInt(record.transactionDate), parseInt(record.referenceDate), parseFloat(record.value), detailsObj, record.id, parseInt(record.creationDate), parseInt(record.lastModifiedDate));
                this._transactions.push(transaction);
            }
        }
        catch (error) {
...
class Transaction extends entity_1.Entity {
    constructor(name, description, type, agent, transactionDate, referenceDate, value, details, // Alterado para Details | null
    ...
    getInfo() {
        return Object.assign(Object.assign({}, super.getInfo()), { type: this._type, agent: this._agent, transactionDate: this._transactionDate, referenceDate: this._referenceDate, value: this._value, details: this._details ? this._details.getInfo().products : [] });
    }
}
